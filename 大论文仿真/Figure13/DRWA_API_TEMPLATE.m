...
hold on;
plot(t,en_top,'Color','g','LineWidth',lineWidth);
hold on;
plot(t,en_bottom,'Color','r','LineWidth',lineWidth);
hold on;
plot(t,en_median,'Color','k','LineWidth',lineWidth);
...
%   axis tight; % 设置坐标紧凑型
    ...
    set(gca,'YTickLabel',10*get(gca,'YTick'))  % 乘10^6，让图像的纵坐标显示的是微米量级
    if nargin - 3== 2
        return;
    end
    %% 第3幅图
