...
%% 一阶低频
wname = "db4";  % Daubechies 4-tap
[cA,cB] = dwt(p,wname);  % cA为近似分量，我的理解就是低频分量，cD细节分量，我的理解是高频分量
xrec_low = idwt(cA,zeros(size(cA)),wname);  % 实际上这里应该是X=idwt(cA,cD,'wname')，我这里只保留了低频分量，相当于一个低通滤波器了

%% 二阶低频
[cC,cD] = dwt(xrec_low,wname);
xrec_low2 = idwt(cC,zeros(size(cC)),wname); 
subplot(9,1,2);
plot(xrec_low2);
title("二阶低频a2n");

%% 一阶高频
xrec_high = idwt(zeros(size(cA)),cB,wname);  % 
subplot(9,1,3);
plot(xrec_high);
title("一阶高频");
...
plot(diffp);
title("differential");
[pks1,locs1] = findpeaks(diffp,'minpeakheight',0.6);
[pks2,locs2] = findpeaks(-diffp,'minpeakheight',0.6);

spatial_filter = zeros(1,N);
locs = sort([locs1, locs2]);
for i=1:N
    if ismember(i,locs)
        spatial_filter(i-5:i+5) = 1;
    end
end
subplot(9,1,5);
plot(spatial_filter);
...
subplot(9,1,6);
xrec_high = xrec_high .* spatial_filter;
plot(xrec_high);
title("一阶高频滤波d1fn");


[cA,cB] = dwt(xrec_high,wname);

%% 二阶高频
subplot(9,1,7);
xrec_high2 = idwt(zeros(size(cD)),cD,wname);  % 二阶高频
xrec_high2 = xrec_high2 .* spatial_filter;
plot(xrec_high2);
title("二阶高频滤波d2fn");

%% 成分相加
subplot(9,1,8);
p_ = xrec_high + xrec_high2 + xrec_low2;
plot(p_);
hold on ;
% plot(xrec_low2);
title("a2n+df1n+df2n");
