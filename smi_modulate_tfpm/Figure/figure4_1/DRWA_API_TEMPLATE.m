function DRWA_API_TEMPLATE(varargin)
    ...
    len = length(F);
    TF = abs(TF);
    mesh(T,F(len/2-700:len/2+700,:),TF(len/2-700:len/2+700,:));
%     mesh(T,F,abs(TF));
    colormap Jet;
    view(0,90);
...
%   axis([0 0.08 -20*10^-9 20*10^-9]);  % 设置具体横坐标和纵坐标的范围[x1,x2,y1,y2]
    ...
    plot(t(2000:8000),p11(2000:8000),'color','#D95319','LineStyle','-');
    hold on;
    plot(t(2000:8000),p1(2000:8000),'color','#0072BD','LineStyle','-');
    % 横纵坐标轴+图框的粗细、大小、线宽
    set(gca,'LineWidth',lineWidth,'FontName',fontName,'FontWeight',fontWeight,'FontSize',fontSize);
    xlabel('Time(ms)')
    ylabel('Amp.(a.u)')
