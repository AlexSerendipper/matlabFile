function DRWA_API_TEMPLATE(varargin)
    ...
    %% 第2幅图
    axes('Position',[0.3635 0.6053 0.237 0.289]); 

    len = length(F);
    TF = abs(TF);
    mesh(T(41:140),F(len/2+375:len/2+625,:),TF(len/2+375:len/2+625,41:140));
    view(0,90);
    % 横纵坐标轴+图框的粗细、大小、线宽
    set(gca,'LineWidth',lineWidth,'FontName',fontName,'FontWeight',fontWeight,'FontSize',fontSize);
%     xlabel('Time(ms)')
...
%     title('(b)', 'Units', 'normalized','FontWeight','bold', 'color', 'white', 'Position', [0.098148568945179,0.683091931244538,0] ,'FontSize', 16);
    set(gca,'YTickLabel',10^-3*get(gca,'YTick'))  % 乘10^6，让图像的纵坐标显示的是微米量级
    set(gca,'XTickLabel')  % 乘10^6，让图像的纵坐标显示的是微米量级
    ...
    len = length(F1);
    TF1 = abs(TF1);
    mesh(T1,F1(len/2-875:len/2+875,:),TF1(len/2-875:len/2+875,:));
    view(0,90);
    ...
    grid on;
    axis tight; 
    title('(b)', 'Units', 'normalized','FontWeight','bold', 'color', 'white', 'Position', [0.08354821575937,0.862622743084944,0] ,'FontSize', 16);
    set(gca,'YTickLabel',10^-3*get(gca,'YTick'))  % 乘10^6，让图像的纵坐标显示的是微米量级
    set(gca,'XTickLabel')  % 乘10^6，让图像的纵坐标显示的是微米量级
    
    %% 第4幅图
    axes('Position',[0.3635 m1(2) 0.237 0.289]);  axis tight;  % 设置图像位置 + 坐标紧凑型 
    len = length(F1);
    TF1 = abs(TF1);
    mesh(T1(41:140),F1(len/2+375:len/2+625,:),TF1(len/2+375:len/2+625,41:140));
    view(0,90);
...
%     ylabel('Amp.(a.u)')
    grid on;
    axis tight; 
%     title('(c)', 'Units', 'normalized', 'Position', [0.098148568945179,0.683091931244538,0] ,'FontSize', 16);
    ...
    plot(f1,amp1,'LineWidth',lineWidth);
    ...
    plot(f2,amp2,'LineWidth',lineWidth);
