...
%      set(gcf,'Position',[200.2,193,1149.6,463.2]);
    ...
    %% 第1幅图
    axes('Position',[0.06,0.096264367816092,0.401,0.837735632183908]);  % 设置图像位置 
    mesh(T,F,abs(TF));
    colormap Jet;
    view(0,90);
...
%   axis([0 0.08 -20*10^-9 20*10^-9]);  % 设置具体横坐标和纵坐标的范围[x1,x2,y1,y2]
    title('(a)', 'Units', 'normalized', 'FontWeight','bold', 'color', 'white', 'Position', [0.056286238224517,0.929241852487136,0] ,'FontSize', 16);
...
%     end
    ...
    plot(t,Ltt'-Lt_reconstruct,'LineWidth',lineWidth,'color','#0072BD');
    legend("RMS=45.4nm");
