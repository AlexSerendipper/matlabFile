...
%% 取出驼峰区来翻倍
p_part = p(338:664);
p_part = SMI_API_EVENPOWER(2,p_part);
plot(p_part);
p_part = -1 + (p_part - min(p_part))/(max(p_part)-min(p_part))*(1+1);
p(338:664) = p_part;
...
% end
for i = 2:length(Lt_reconstruct_beforeinternp)
    if ismember(trigger(i),338:664) && direction(trigger(i))==direction(trigger(i-1))
        Lt_reconstruct_beforeinternp(i) = Lt_reconstruct_beforeinternp(i-1) + step/16*direction(trigger(i));
    elseif ismember(trigger(i),338:664) && direction(trigger(i))~=direction(trigger(i-1))
        Lt_reconstruct_beforeinternp(i) = Lt_reconstruct_beforeinternp(i-1);
    elseif direction(trigger(i))==direction(trigger(i-1))
        Lt_reconstruct_beforeinternp(i) = Lt_reconstruct_beforeinternp(i-1) + step*direction(trigger(i));
    elseif direction(trigger(i))~=direction(trigger(i-1))
