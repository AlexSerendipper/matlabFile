...
% f = @(x, k) x.^k ./ (x.^k + (1-x).^k);
% a = k/2;
f = @(x, k) x.^(k/8*k) ./ (x.^(k/8*k) + (1-x).^k);
% 设置 x 范围
x = linspace(0, 1, 100);

plot(x, f(x, 0.1), 'LineWidth', lineWidth);
hold on;
plot(x, f(x, 0.5), 'LineWidth', lineWidth);
hold on;
plot(x, f(x, 8), 'LineWidth', lineWidth);
hold on;
plot(x, f(x, 16), 'LineWidth', lineWidth);
hold on;
plot(x, f(x, 32), 'LineWidth', lineWidth);
hold on;
plot(x, f(x, 64), 'LineWidth', lineWidth);
hold on;
plot(x, f(x, 128), 'LineWidth', lineWidth);
hold on;
plot(x, f(x, 256), 'LineWidth', lineWidth);
hold on;
legend("k=0.1","k=0.5","k=8","k=16","k=32","k=64","k=128","k=256");
