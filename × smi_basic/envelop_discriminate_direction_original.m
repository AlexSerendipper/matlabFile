% Speckle affected fringe detection based on three envelope extraction for self-mixing displacement measurement.2017.Optics Communications
% 利用包络法实现条纹倾斜方向的判断以及实现正确的条纹计数,理论适用范围为：0.01<C<4.6。实际使用中最稳定范围是：0.05<= C <=3.6,     
% 在实际中C>3.6时，跳变点去除出现问题。可以通过更改阈值适应（这个设置与C值N值、A值、f值都有关系，所以需要找到一个稳定的函数关系~~）
% 在C<0.05时，sign函数出现问题（envelop_middle）(可能是由于diff函数我补的那个值？)
...
%% main function4(correct_direction_fringe_counting) ——难点2：这个符号函数有N个点，要矫正方向，难点在于把location3中其余点按顺序补成NaN才能和sign相乘         
...
top3_convert = direction_append1 .* top3_insert;
