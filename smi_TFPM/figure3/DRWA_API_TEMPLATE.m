function DRWA_API_TEMPLATE(varargin)
    ...
    len = length(F1);
    TF1 = abs(TF1);
    mesh(T1,F1(len/2-xxx:len/2+xxx,:),TF1(len/2-xxx:len/2+xxx,:));
%     mesh(T1,F1,abs(TF1));
view(0,90);
    % 横纵坐标轴+图框的粗细、大小、线宽
...
%   axis([0 0.08 -20*10^-9 20*10^-9]);  % 设置具体横坐标和纵坐标的范围[x1,x2,y1,y2]
    ...
    len = length(F2);
    TF2 = abs(TF2);
    mesh(T2,F2(len/2-xxx:len/2+xxx,:),TF2(len/2-xxx:len/2+xxx,:));
%     mesh(T2,F2,abs(TF2));
view(0,90);
    % 横纵坐标轴+图框的粗细、大小、线宽
    set(gca,'LineWidth',lineWidth,'FontName',fontName,'FontWeight',fontWeight,'FontSize',fontSize);
...
%     box off;  %关闭右面和上面的坐标轴
    ...
    len = length(F3);
    TF3 = abs(TF3);
    mesh(T3,F3(len/2-xxx:len/2+xxx,:),TF3(len/2-xxx:len/2+xxx,:));
%     mesh(T3,F3,abs(TF3));
view(0,90);
    % 横纵坐标轴+图框的粗细、大小、线宽
    set(gca,'LineWidth',lineWidth,'FontName',fontName,'FontWeight',fontWeight,'FontSize',fontSize);
    ...
    grid on;
    axis tight; 
    title('(c)', 'Units', 'normalized','FontWeight','bold','color', 'white', 'Position', z ,'FontSize', 16);
    set(gca,'YTickLabel',10^-3*get(gca,'YTick'))  % 乘10^6，让图像的纵坐标显示的是微米量级
    if nargin - 3  == 3
        return;
    end
