...
function DRWA_API_TEMPLATE(varargin)
    ...
    for i = 1:length(files)
     ...
     % 根据需要设置最舒服的图窗大小
     set(gcf,'Position',[81,101,1036.666666666667,344]);
    ...
    plot(t,p_init1,'LineWidth',lineWidth);
...
%   set(gca,'xtick',[],'ytick',[],'xcolor','w','ycolor','w')  % 不显示x轴和y轴
    ...
    plot(t,p_init2,'LineWidth',lineWidth);
...
%   axis tight; % 设置坐标紧凑型
    ...
    mesh(T1,F1(len/2-100:len/2+100,:),TF_curb1(len/2-100:len/2+100,:));
%     mesh(abs(TF_curb)); 
    ...
    len = length(F2);
    TF_curb2 = abs(TF_curb2);
    mesh(T2,F2(len/2-100:len/2+100,:),TF_curb2(len/2-100:len/2+100,:));
%     mesh(abs(TF_curb)); 
    view(0,90); % 设置初始视角为俯视角
