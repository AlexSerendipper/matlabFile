%% 对direction>0的信号段进行去直流

%% 全局变量
clc;
clear all;
close all;
fs = 200000;  % 采样率，即fs(s)采一个点。采样率越大，相同间隔下点数越多
N = 4000; % 点数越多信号越长
fv = 200;  % 震动频
alpha = 5;
...
% second = p(252:751);
% plot(second);
% g_ = eliminate_dc(second);
% subplot(4,1,4);
% g_ = -1 + (g_ - min(g_))/(max(g_)- min(g_)) * 2;
% plot(g_);
%% 去直流分量
figure(2);
% 重置方向发生变化的向量direction_seg1
direction_seg1 = [];
...
for i = 1 : 2 : length(direction_seg2)
    p_ = p(direction_seg2(i):direction_seg2(i+1));
    g_ = eliminate_dc(p_);
%     g_ = -1 + (g_ - min(g_))/(max(g_)- min(g_)) * 2;
    p(direction_seg2(i):direction_seg2(i+1)) = g_;
end
subplot(2,1,1);
hold on;
plot(p);
