...
%     set(gca,'YTickLabel',10^6*get(gca,'YTick'))  % 乘10^6，让图像的纵坐标显示的是微米量级
    ...
    % 第2幅图
    axes('Position',[0.538,0.775,0.401,0.16]);   % 设置图像位置 + 坐标紧凑型
    
    plot(fshift,amp1); grid on;  % 作图 + 网格
    axis tight;
    ylabel('amp.(a.u.)');
    xlabel('Freq.(Khz)'); title('(b)', 'Units', 'normalized','FontSize', 16, 'Position', [0.035 0.7]);  % y轴注释 + 左上角角标，小标位置是相对当前图而言
    xlim([-250000 250000])
