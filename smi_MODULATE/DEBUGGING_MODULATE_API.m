...
%% 目前最主要的问题是，如果你去直流了（Lt-mean(Lt)），那条纹计数法影响不大了已经
   % 所以只能用在小振幅，很垃圾
...
clear all;
close all;
fs = 400000;  % 采样率，即fs(s)采一个点。采样率越大，调制频率就要越大，否则会失真，默认采样率10000==>调制频率2000，二者成倍数关系
N = 10000;  % 调制默认值为10000
fv = 80;  % 震动频，调制默认值为10
alpha = 5;
n = 0 ;  % 翻倍次数，艹啊。2和3一样
...
pp_ = takeHarmonicComponent(p_,6500,7500);
subplot(9,1,3);
amp2 = abs(pp_) * 2 / N ;
plot(fshift,amp2);
...
pp_ = takeHarmonicComponent(p_,8500,9500);
subplot(9,1,5);
