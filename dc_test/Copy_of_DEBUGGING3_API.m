...
%% 这里的想法是从去直流入手,但还是从时域上操作信号，减去信号的平均值
...
direction_seg = direction_seg2;  % 不加上这句就是仅条纹远离时去直流
...
% 5 仅去上条纹
% direction_seg = [1, loc_diffp_seg(1),loc_diffp_seg(4),loc_diffp_seg(5),loc_diffp_seg(8),length(direction)];
% direction_seg = [1, sort([direction_seg,direction_seg+1]), length(direction)];  % 这行注释掉就是仅去上条纹，不注释就是仅上条纹单独去~
...
plot(p);
subplot(4,1,2);
temp  = p(direction_seg2(1):direction_seg2(2));
% temp = wextend('1D','sym',temp,20);  % 0数据延拓
temp = wextend('1D','zpd',temp,20);  % 0数据延拓
plot(temp);
...
hold on;
% plot(direction);
p(direction_seg2(1):direction_seg2(2)) = temp(21:521);
hold on;
plot(p);
...
padding =  windowLength;
while judge
    if mod(2 * padding + N - overlapLength,windowLength - overlapLength) == 0
        judge = false;
    else
        padding = padding - 1;
    end
...
p = wextend('1D','sym',p,padding);  % 对称数据延拓
p = wextend('1D','sym',p,padding);  % 对称数据延拓
% p = [zeros(1,padding) zeros(1,padding) p zeros(1,padding) zeros(1,padding)];
% p = wextend('1D','zpd',p,padding);  % 0数据延拓
% p = wextend('1D','zpd',p,padding);  % 0数据延拓
p_afterpadding = p;


%% padding后尝试操作,结果表明，抑制前对信号整体右移5个单位，一样能起到好的效果，芜湖

% 整体右移
p = p(padding+5:padding+padding+N+padding+4);
...
% p = 2 * (p - -abs(hilbert(-p)))./(abs(hilbert(p)) - -abs(hilbert(-p))) - 1;  % 葛兄归一化，看看条纹倾斜去掉了没
...
% 
% subplot(4,1,2);
% temp  = p_init2(direction_seg2(1):direction_seg2(2));
% temp = wextend('1D','sym',temp,40);  % 0数据延拓
% plot(temp);
% 
% subplot(4,1,3);
% plot(p_beforepadding,'k');
% hold on;
% % plot(direction);
% p_init2(direction_seg2(1):direction_seg2(2)) = temp(1:501);
% hold on;
% plot(p_init2)
