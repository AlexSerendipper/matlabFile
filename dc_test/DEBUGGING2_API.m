...
% %% 这里的想法是从去直流入手,但还是从频域上操作信号，debugging3从时域入手，减去信号的平均值
% 
% %% 全局变量
% clc;
% clear all;
...
% 
% for i = 1 : 2: length(direction_seg) 
%     p_ = p(direction_seg(i):direction_seg(i+1));
%     g_ = eliminate_dc(p_);
%     % g_ = -1 + (g_ - min(g_))/(max(g_)- min(g_)) * 2;
%     p(direction_seg(i):direction_seg(i+1)) = g_;
% end
% 
% 
