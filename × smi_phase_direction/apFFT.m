...
%% 抽样信号定义块
N=128;
n=-N+1:N+1;  % 在我看来这是实际需要的采样点，我们实际需要的信号更长，就是两倍的信号-1的长度拉
...
% 这个信号里有10、20、30、40、50五个频率

%% 窗的定义块
y1=s(N:2*N-1);  % N长的数据送入FFT处理（这就是两倍长度信号的后一段，）
y2=s(1:2*N-1);  % 2N-1长的数据送入APFFT处理（这就是两倍长度信号的整段，）
...
p_apfft=mod(angle(y222_fft)*180/pi,360);  % APFFT相频
%APFFT的相频和幅频表达
