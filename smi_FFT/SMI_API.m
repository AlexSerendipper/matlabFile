...
function [t, lambda, L0, Lt, phi0, p] = SMI_API(fs, N, fv, C, alpha)   % fs为采样率(s)，每秒钟采样多少个点。采样率要比采样点数大的多才能不失真！fs/fv需为整数（✔）N需要为2次幂,否则频谱混叠（✔）N = KM（M为运动周期）
    ...
    A = 50 * lambda / (4 * pi);  % 幅值（✔）
    L0 = 40e-9;  % 外腔距离（✔） 
    Lt = A.* cos(2*pi*fv*t);  % L0为标称位置，外腔长度
