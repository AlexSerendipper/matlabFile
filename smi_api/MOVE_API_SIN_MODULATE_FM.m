...
function [t, lambda, L0, Lt, phi0, p, c] = MOVE_API_SIN_MODULATE_FM(fs, N, C, alpha)   % fs为采样率(s)，每秒钟采样多少个点。采样率要比采样点数大的多才能不失真！N需要为2次幂,否则频谱混叠（✔）N = KM（M为运动周期）
...
%     A2 = 2.4 .* lambda;  % 调制信号幅值（✔），A1+A2 = 4*lambda
    ...
    A1 = 2 .* lambda;
    A2 = 10 ;  % 调制信号幅值（✔），A1+A2 = 2*lambda
    
    fv1 = 200;
    fv2 = 100;  % 调制信号频率（hz）
    L0 = 20 * lambda;  % 外腔距离（✔） 
    Lt = A1 .* sin(2*pi .*(fv1 + A2 .* sin(2*pi*fv2*t)) .*t);  % 振幅调制，其中A2项为调制项
